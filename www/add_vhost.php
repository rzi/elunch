...
if(isset($_POST['vhostdelete']) && (isset($_POST['checkdelete']) && strip_tags(trim($_POST['checkdelete'])) == $_SESSION['passdel'])) {
	if(isset($_POST['virtual_del'])) {
		...
		for($i = 0; $i < $nb ;$i++) { //for b1
			...
			if(strpos($value, ':') !== false) {
				...
				if(in_array($port,$c_ApacheDefine)) {
...
if($virtualHost['nb_Server'] > 0) {
	...
	foreach($virtualHost['ServerName'] as $value) {
		...
		$VhostDefine .= "<li><i>ServerName : </i><span style='color:blue;'>".$value_url."</span>".$UrlPortVH." - <i>Directory : </i>".$virtualHost['documentPath'][$i].$ip."</li>\n";
		if($value != 'localhost')
...
/* If form submitted */
if (isset($_POST['submit']) && !$errors && (isset($_POST['checkadd']) && strip_tags(trim($_POST['checkadd'])) == $_SESSION['passadd'])) {
